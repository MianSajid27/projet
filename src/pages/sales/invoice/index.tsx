...
export default function TypesPage() {
  ...
  React.useEffect(() => {
    ...
    startDate +
    '&end_date=' +
    endDate+ '&per_page=' + listPerPage).then((result) => {
      if (result) {
  ...
  React.useEffect(() => {
    ...
    GetFunctionBDetail('/business-details', token).then((result) => {
      console.log(result, 'result ignite');
  ...
  React.useEffect(() => {
    GetFunction('/sell?order_by_date=desc'+ '&per_page=' + listPerPage).then((result) => {
      setMetaData(result.meta);
      setDummyArr(result.data);
      setNewArr(result.data);
      setloadingData(false);
      var perPage= result?.meta?result.meta.total:result.data.length
      GetFunction('/sell?per_page=' +perPage).then((result) => {
  ...
  React.useEffect(() => {
    let token = localStorage.getItem('user_token');
    GetFunctionBDetail('/business-details', token).then((result) => {
      console.log('enable', result.data.enable_flextock);
  ...
  const onApplyFilter = () => {
    ...
    GetFunction(
      '/sell?contact_id=' +
        ...
        startDate +
        '&end_date=' +
        endDate+ '&per_page=' + listPerPage  +"&order_by_date=desc"
    ).then((result) => {
      setMetaData(result.meta);
      setDummyArr(result.data);
      setNewArr(result.data);
  ...
  const ChangePagination = (current) => {
    ...
    GetFunction(
      '/sell?contact_id=' +
        ...
        filterData.status +
        '&page=' +
        current + '&per_page=' + listPerPage +"&order_by_date=desc"
    ).then((result) => {
      setMetaData(result.meta);
      setDummyArr(result.data);
      setNewArr(result.data);
  ...
  const excelExportData = (NewStart, NewEend, total) => {
    GetFunction('/sell?&per_page=' + total).then((result) => {
      let newProductList = result?.data.map((item: any) => {
        return {
          Customer_Name: item?.contact.name,
          Date: item?.transaction_date,
