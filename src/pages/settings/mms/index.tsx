...
import MsMList from '@/components/mms/mms-list';
...
export default function TypesPage() {
  ...
  return (
    <>
      <Card className="mb-8 flex flex-col">
        <div className="flex w-full flex-col items-center md:flex-row">
          <div className="mb-4 md:mb-0 md:w-1/4">
            <h1 className="text-lg font-semibold text-heading">{t('form:mms')}</h1>
          </div>
          <div className="flex w-full flex-col items-center space-y-4 ms-auto md:flex-row md:space-y-0 xl:w-1/2">
            <Search onChangeearchVal={filterBySearch} />
            <LinkButton
              href={Routes.mms.create}
              className="h-12 w-full md:w-auto md:ms-6"
            >
              <span className="block md:hidden xl:block">+ {t('form:add-mms')}</span>
      ...
      <MsMList list={newArr} isUpdate={true} isView={true} />
    </>
  );
}
