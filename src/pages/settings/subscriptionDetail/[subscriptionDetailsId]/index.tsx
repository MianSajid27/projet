...
import { GetBusinessDetail, GetFunction, GetFunctionBDetail } from '@/services/Service';
...
export default function OrderDetailsPage() {
        ...
        if (url) {
          ...
          GetFunction('/pabbly/subscriptions-and-related-addons').then(
            (result) => {
              ...
              let total = 0
              result?.data.customer_subscriptions[0].addons?.map((item: any) => {
                total += item.price


              })

              let newTotal = total + result?.data.customer_subscriptions[0].plan.price
  ...
  React.useEffect(() => {
   ...
   console.log(PackagesList,'packageLIst');
  
  const businessNameItem = PackagesList[0]?.custom_fields?.find(item => item.name === 'business_name');
  if (loadingData) return <Loader text={t('common:text-loading')} />;
  return (
    <>
      <Card>
        {businessNameItem && <Label className="text-lg ">{businessNameItem.value}</Label> }
     
        <div className="text-sm text-slate-600">{PackagesList[0]?.email_id}</div>
      </Card>
        ...
        <div className='flex lg:col-span-8'>
          <Card className="mt-5 w-full ">
            ...
            <div className="flex justify-between border-t-2 border-dashed pt-3 pb-3">
              ...
              <div className=" text-sm text-slate-600"> {PackagesList[0]?.plan?.plan_name}</div>
              <div className="text-sm flex  text-slate-600 "> {PackagesList[0]?.plan?.price} {currency?.symbol}</div>
            </div>
            {PackagesList &&
              PackagesList[0]?.addons?.map((res, i) => {
                return (
                  <div key={i} className="flex justify-between border-t-2 border-dashed pt-3 pb-3">
                    ...
                    <div className="text-sm text-slate-600">{res.price} {currency?.symbol}</div>
                  </div>
                );
              })}
            <div className="flex justify-between border-t-2 border-dashed pt-3 pb-3">
              <div className="text-md font-semibold text-accent-600">{t('common:subscription-amount')}:</div>
