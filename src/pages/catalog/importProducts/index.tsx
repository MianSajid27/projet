...
export default function TypesPage() {
  ...
  const getListFun = () => {
    ...
    GetFunction('/import-requests?type=product').then((result) => {
      setData(result.data);
  ...
  const handleDownloadClick = () => {
    const header =
      'WEIGHT\tRACK\tROW\tPOSITION\tIMAGE\tPRODUCT\tDECRIPTION\tCUSTOM FILED 1\tCUSTOM FIELD 2\tCUSTOM FIELD 3\tCUSTOM FIELD 4\tNOT FOR SELLING(1=yes 0=No)\tPRODUCT LOCATION\n'; // Replace with your desired header
    const blob = new Blob([header], { type: 'application/vnd.ms-excel' });
    saveAs(blob, 'import_products_csv_template.xls');
  };
