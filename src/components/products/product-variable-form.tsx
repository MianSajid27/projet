...
export default function ProductVariableForm({
  ...
  const onConfirmDelet = () => {
    ...
    DashboardGetFun('/product/delete/' + deleteImgId).then((result) => {
      if (result.success) {
        initialValues.variation_options[deleteImgIndex].media =
          initialValues.variation_options[deleteImgIndex].media.filter(
  ...
  const handleSelectChange = (index, value) => {
    ...
    if (editId) {
      if (Array.isArray(value)) {
        ...
        const updatedValue = value.reduce((acc, option) => {
        ...
        if (!currentValue.some((item) => item?.id === value?.id)) {
          ...
          const filteredValue = currentValue.filter((item) => item?.id !== value?.id);
