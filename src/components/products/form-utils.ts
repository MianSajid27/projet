...
export function filterAttributes(attributes: any, variations: any, editId: any = null, checkedVariation: any = null) {
  // console.log('attribtues and variaionts',attributes + variations);
...
export function getSelectedCartesianProduct(values: any , options :any) {
  const attrKeys =  Array.isArray(options.options) ?   Object.keys(arrayToObject(options.options)) : Object.keys(options.options);
...
export function getCartesianOption(values: any , options:any ,editId:any) {
  ...
  if(editId){
    ...
    ?.map((v: any) =>
      (v?.variation_template_id)
    )
    .filter((i: any) => i !== undefined);
  }else{
    var formattedValues = values
      ?.map((v: any) =>
        v?.value?.map((a: any) => ( v?.attribute?.id))
      )
      .filter((i: any) => i !== undefined);
  }
