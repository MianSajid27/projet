...
export default function CreateOrUpdateTypeForm(initialValue: any) {
            ...
            GetFunction(
                ...
                newvariable?.map((data, index) => {
                    data?.variation_options.map((res, i) => {
                        ...
                        const final_Variation = res.variation_location_details.filter((item) => {
                            return item.location_id == locationID;
                        })
                        let productLocation =res.variation_location_details.length ? ' (' +final_Variation[0]?.location_id+ ')' :'';
    ...
    React.useEffect(() => {
        console.log(tableArray, 'table table');
    ...
    const handleSelect = (event, product) => {
        setChecked([]);
        // console.log(event.target.id,'event event');
    ...
    const removeHalder = (id, index) => {
        ...
        const updatedItems = tableArray.filter((item) => item?.variation_location_details[0]?.id!== id);
                ...
                <div className=" mt-5">
                    {finalArray.map((res, index) => (
                        ...
                        <div key={index}>
                            ...
                            >
                                <div className="font-bold py-2 text-heading pl-6">{res.name}</div>
                                <div className="flex py-2 px-2 text-sm items-center">
                                    ...
                                    <div className="flex w-full justify-between">
                                        ...
                                        <div className="flex my-1 ">
                                            <Input
                                                ...
                                                onChange={(event) => handleInputChange(index, event, res)}
                                                id="numberInput"
                                                onInput={validateInput}
                                            />
                                        </div>
                                    ...
                                    <div className="pt-5 flex  ms-auto">
                                        <button
                                            ...
                                            onClick={() => removeHalder(res.variation_location_details[0]?.id, index)}
                                        >
                                            ...
                                            <div className="flex">
                                                <div key={index}>
                                                    <input
                                                        ...
                                                        name={res.name}
                                                        id={res.id}
                                                        onChange={(event) => { handleSelect(event, res) }}
                                                    />
                                                </div>
