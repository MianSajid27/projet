...
export function getItemSubscripton(items: Subscription[], id: Subscription['id']) {
  return items.find((item) => item.id === id);
}
...
export function removeItemSubscription(items: Subscription[], id: Subscription['id']) {
  return items.filter((existingItem) => existingItem.id !== id);
}
...
export const calculateUniqueItemsSubscription = (items: Subscription[]) => items.length;
